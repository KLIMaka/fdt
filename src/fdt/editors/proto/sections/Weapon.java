...
public class Weapon extends Composite implements IFillSection {
	...
	public void setup() throws Exception {
		String[] anims = { "None", "Knife (D)", "Club (E)", "2Hnd Club (F)", "Spear (G)", "Pistol (H)", "Uzi (I)", "Rifle (J)", "Laser (K)", "Minigun (L)",
				"Rocket Launcher (M)" };
		for (String a : anims)
			m_anim.add(a);
		...
		String[] atts = { "Stand", "Throw punch", "Kick leg", "Swing", "Thrust", "Throw", "Single", "Burst", "Continous" };
		for (String at : atts) {
			m_at1.add(at);
			m_at2.add(at);
		}
